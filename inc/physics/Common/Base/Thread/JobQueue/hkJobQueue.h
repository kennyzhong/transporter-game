...
struct hkJobHelperBaseClass
{
	public:
		...
		enum hkJobBaseId
		{
			...
			HK_BEHAVIOR_JOB_BASE		=  80,
			HK_END_JOB_BASE				=  100		// just a marker marking the end of the available job id range
		};
...
class hkJobQueue
{
	public:
		...
		enum JobType
		{
			...
			JOB_TYPE_CPU_MASTER		= 2,	// special thread currently used only for multithreaded toi solving
#if defined (HK_PLATFORM_HAS_SPU)
			JOB_TYPE_SPU			= 3,
			JOB_TYPE_HINT_SPU		= 3,
			JOB_TYPE_SPU_CD			= 4,
			JOB_TYPE_HINT_SPU_CD	= 4,
			JOB_TYPE_SPU_CQ			= 4,	// CQ stands for Collision Query
			JOB_TYPE_HINT_SPU_CQ	= 4,
			JOB_TYPE_MAX			= 5,
#else
			JOB_TYPE_HINT_SPU		= 0,
		...
		struct DynamicData
		{
			...
			int m_numMasterThreadsWaitingForJobs;	

			// If this is true, the SPUs won't leave their current physics (collide or integrate) .ELF
			hkBool m_dontReleaseSpus;

			hkQueue< JobQueueEntry > m_jobQueue[JOB_TYPE_MAX];
		};
	...
	public:
		...
		void addJobBatch( hkArray<JobQueueEntry*>& jobs, JobPriority priority, JobType type );
	...
	public:
		...
		hkSemaphoreBusyWait* m_masterThreadWaitSemaphore; // only used by an optional master thread
