	...
	m_numBitsAndFlags(numBits)
{
	if ( m_numWords )
	{
		m_words = hkAllocateChunk<hkUint32>( m_numWords, HK_MEMORY_CLASS_ARRAY );
	}
	else
	{
		m_words = HK_NULL;
...
:   m_words( ptr ),
	m_numWords( ( numBits + 31 ) >> 5 ),
	m_numBitsAndFlags(numBits | DONT_DEALLOCATE_FLAG)
{
}
...
HK_FORCE_INLINE hkBitField::~hkBitField()
{
	if( shouldDeallocate() )
	{
		hkDeallocateChunk( m_words, m_numWords, HK_MEMORY_CLASS_ARRAY );
	}
}
...
HK_FORCE_INLINE const hkUint32 hkBitField::getWord( int wordIndex ) const
{
	HK_ASSERT( 0x38a87b3b, wordIndex >= 0 && wordIndex < m_numWords );

	return m_words[wordIndex];
}

HK_FORCE_INLINE const int hkBitField::getNumWords() const
{
	return m_numWords;
}
