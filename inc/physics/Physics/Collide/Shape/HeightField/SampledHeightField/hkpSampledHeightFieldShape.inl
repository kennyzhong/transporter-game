...
#ifdef HK_ENABLE_HEIGHTFIELDS_ON_SPU
#include <Physics/Dynamics/World/Simulation/Multithreaded/Spu/hkpSpuConfig.h>
extern hkSpu4WayCache* g_SpuCollideUntypedCache;
#endif

HK_FORCE_INLINE hkReal hkpSampledHeightFieldShape::getHeightAtImpl( const hkpSampledHeightFieldShape* hf, int x, int z )
{
#ifndef HK_PLATFORM_SPU
	return hf->getHeightAt( x, z );
#else

#ifdef HK_ENABLE_HEIGHTFIELDS_ON_SPU
	// Example SPU use case: DMA the pre-sampled value
	HK_ASSERT( 0, hf->m_sampledHeights );
	hkUlong addrPpu = reinterpret_cast<hkUlong>(hf->m_sampledHeights) + ( z*hf->m_xRes + x ) * sizeof(hkReal);
	const int mask  = ~(HK_SPU_AGENT_SECTOR_JOB_MAX_UNTYPED_CACHE_LINE_SIZE-1);
	hkUlong addrAligned = addrPpu & mask;
	hkUlong alignedDataSpu = (hkUlong)g_SpuCollideUntypedCache->getFromMainMemoryInlined( (const void*)addrAligned , HK_SPU_AGENT_SECTOR_JOB_MAX_UNTYPED_CACHE_LINE_SIZE, HK_MOPP_DEFAULT_DMA_GROUP, true );
	return *reinterpret_cast<const hkReal*>( alignedDataSpu + (addrPpu & ~mask) );	
#else
	HK_ERROR( 0x26d8e6a1, "No implementation of getHeightAt() for SPU" );
	return 0;
#endif
...
HK_FORCE_INLINE hkBool hkpSampledHeightFieldShape::getTriangleFlipImpl( const hkpSampledHeightFieldShape* hf )
{
#ifndef HK_PLATFORM_SPU
	return hf->getTriangleFlip();
#else
#ifdef HK_ENABLE_HEIGHTFIELDS_ON_SPU
	// Example SPU use case: DMA the pre-sampled value
	HK_ASSERT( 0, hf->m_sampledFlip );
	hkUlong addrPpu = reinterpret_cast<hkUlong>(hf->m_sampledFlip);
	const int mask  = ~(HK_SPU_AGENT_SECTOR_JOB_MAX_UNTYPED_CACHE_LINE_SIZE-1);
	hkUlong addrAligned = addrPpu & mask;
	hkUlong alignedDataSpu = (hkUlong)g_SpuCollideUntypedCache->getFromMainMemoryInlined( (const void*)addrAligned , HK_SPU_AGENT_SECTOR_JOB_MAX_UNTYPED_CACHE_LINE_SIZE, HK_MOPP_DEFAULT_DMA_GROUP, true );
	return *reinterpret_cast<const hkBool*>( alignedDataSpu + (addrPpu & ~mask) );
#else
	HK_ERROR( 0x26d8e6a2, "No implementation of getTriangleFlip() for SPU" );
	return false;
#endif
