...
public:
	...
	*/
	...
	HK_FORCE_INLINE void setFloatSlotValue (int floatSlotIdx, hkReal value);
		...
		/// applications is going to repeatedly access most of these Local Space transforms (without modifying them in Model Space)
	void syncLocalSpace () const;

		/// Ensures all Local Space transforms stored are in sync (since they are evaluated lazily, they may not always be in sync). It is useful if the
		/// applications is going to repeatedly access most of these Model Space transforms (without modifying them in Local Space)
	...
	*/
	...
	HK_FORCE_INLINE hkArray<hkReal>& writeAccessFloatSlotValues();
