...
/// hkaInterleavedSkeletalAnimation meta information
extern const class hkClass hkaInterleavedSkeletalAnimationClass;

/// The information needed to construct an hkSimpleAnimation (An interleaved
/// uncompressed stream of Bone Transforms)
class hkaInterleavedSkeletalAnimation : public hkaSkeletalAnimation
{
	public:
		...
		HK_DECLARE_REFLECTION();
			...
			/// Default constructor
		inline hkaInterleavedSkeletalAnimation() :
			m_transforms( HK_NULL ), m_numTransforms( 0 ),
			m_floats( HK_NULL ), m_numFloats( 0 )
		{
			m_type = hkaSkeletalAnimation::HK_INTERLEAVED_ANIMATION;
		}
	...
	public:
		...
		HK_FORCE_INLINE hkaInterleavedSkeletalAnimation( hkFinishLoadedObjectFlag flag ) : hkaSkeletalAnimation(flag) {}
};
