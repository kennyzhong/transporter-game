...
/// hkaSplineSkeletalAnimation meta information
extern const class hkClass hkaSplineSkeletalAnimationClass;


/// Compresses animation data using a spline approximation.
/// See Animation Compression section of the Userguide for details.
class hkaSplineSkeletalAnimation : public hkaSkeletalAnimation
{
public:
	...
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_ANIM_COMPRESSED, hkaSplineSkeletalAnimation::TrackCompressionParams );
		HK_DECLARE_REFLECTION();
	...
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_ANIM_COMPRESSED, hkaSplineSkeletalAnimation::AnimationCompressionParams );
		HK_DECLARE_REFLECTION();

		// Per-animation settings
		hkUint16 m_maxFramesPerBlock; ///< Maximum number of frames per block (Typically only used for SPU animations)
		hkBool m_enableSampleSingleTracks; ///< Enable individual tracks to be sampled (can increast asset size)
	...
	struct PerTrackCompressionParams
	{
		...
		hkArray< struct hkaSplineSkeletalAnimation::TrackCompressionParams > m_parameterPalette; ///< Compression settings palette

		/// An array of indices into the palette above 
		hkArray<int> m_trackIndexToPaletteIndex; ///< Map for each track to the chosen compression parameter palette
		hkArray<int> m_floatTrackIndexToPaletteIndex; ///< Map for each float track to the chosen compression parameter palette
	};

	/// Constructor with uniform compression settins for each track
	hkaSplineSkeletalAnimation(
		const hkaInterleavedSkeletalAnimation& raw,
...
public:
	...
	HK_FORCE_INLINE hkaSplineSkeletalAnimation( hkFinishLoadedObjectFlag flag ) :
		hkaSkeletalAnimation( flag ),
		m_blockOffsets( flag ),
...
private:
	...
	void initialize( const hkaInterleavedSkeletalAnimation& raw, const PerTrackCompressionParams& trackParams, const AnimationCompressionParams& animationParams );
...
#include <Animation/Animation/Animation/SplineCompressed/hkaSplineSkeletalAnimation.inl>
