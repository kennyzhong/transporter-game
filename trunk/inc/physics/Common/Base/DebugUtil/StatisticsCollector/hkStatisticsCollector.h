...
class hkStatisticsCollector
{
	public:
		...
		HK_FORCE_INLINE void addPointerMap( const char* name, StatisticClass statisticClass, const hkPointerMap<KEY,VALUE>& map )
		{
			addChunk( name, statisticClass, map.getMemStart(), map.getSize() * hkSizeOf(KEY), map.getMemSize() );
		}
