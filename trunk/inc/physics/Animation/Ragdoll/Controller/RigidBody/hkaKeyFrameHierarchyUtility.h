...
class hkaKeyFrameHierarchyUtility 
{
	...
	public:
		...
		struct ControlData
		{
			...
			hkReal m_hierarchyGain; //+default(0.17f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0)
			...
			hkReal m_velocityDamping; //+default(0.0f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0)
			...
			hkReal m_accelerationGain; //+default(1.0f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0)
			...
			hkReal m_velocityGain; //+default(0.6f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0)
			...
			hkReal m_positionGain; //+default(0.05f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0)	
			...
			hkReal m_positionMaxLinearVelocity; //+default(1.4f)+hk.RangeRealAttribute(absmin=0.0,absmax=100.0,softmax=10.0)

				/// The orientation difference is scaled by the inverse delta time to compute an angular 
				/// velocity to be applied to the rigid body. The velocity is first clamped to this 
				/// limit before it is applied.
			hkReal m_positionMaxAngularVelocity; //+default(1.8f)+hk.RangeRealAttribute(absmin=0.0,absmax=100.0,softmax=10.0)

				/// To allow for precise matching between keyframes and current position. 
			...
			hkReal m_snapGain; //+default(0.1f)+hk.RangeRealAttribute(absmin=0.0,absmax=1.0) 

				/// See m_snapGain. The linear velocity calculated from the snapGain is clamped to this 
				/// limit before being applied.
			hkReal m_snapMaxLinearVelocity; //+default(0.3f)+hk.RangeRealAttribute(absmin=0.0,absmax=100.0,softmax=10.0)

				/// See m_snapGain. The angular velocity calculated from the snapGain is clamped to this 
				/// limit before being applied.
			hkReal m_snapMaxAngularVelocity; //+default(0.3f)+hk.RangeRealAttribute(absmin=0.0,absmax=100.0,softmax=10.0)
			...
			hkReal m_snapMaxLinearDistance; //+default(0.03f)+hk.RangeRealAttribute(absmin=0.0,absmax=10.0,softmax=1.0)
				...
				/// by sqrt( maxDistane/currentDistance ).
			hkReal m_snapMaxAngularDistance; //+default(0.1f)+hk.RangeRealAttribute(absmin=0.0,absmax=10.0,softmax=1.0)
