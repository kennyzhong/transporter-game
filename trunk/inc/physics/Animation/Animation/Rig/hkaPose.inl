	...
	: m_skeleton(skeleton), m_modelInSync(false), m_localInSync (false)
{
	...
	m_floatSlotValues.setSize(m_skeleton->m_numFloatSlots);
...
int hkaPose::getRequiredMemorySize (const hkaSkeleton* skeleton)
{
	...
	const int totalSize =  numBones * ( 2 * hkSizeOf(hkQsTransform) + hkSizeOf(hkaPoseFlag) )
		+ skeleton->m_numFloatSlots * hkSizeOf(hkReal);
...
hkaPose::hkaPose(const hkaSkeleton* skeleton, void* preallocatedMemory)
	:	m_skeleton  (skeleton), 
		...
		m_modelInSync (false),
		m_localInSync (false),
		m_floatSlotValues (reinterpret_cast<hkReal*> (m_boneFlags.begin() + skeleton->m_numBones), skeleton->m_numFloatSlots, skeleton->m_numFloatSlots)
{
...
hkArray<hkReal>& hkaPose::writeAccessFloatSlotValues()
{
	return m_floatSlotValues;
}
