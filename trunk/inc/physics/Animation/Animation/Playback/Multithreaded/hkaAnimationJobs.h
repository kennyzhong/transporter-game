...
// VERY CONSERVATIVE 75k max space for animation buffer. 
// Use either:
// hkaSkeletalAnimation::getMaxSizeOfCombinedDataChunks() for sample-only jobs , or
...
const int DEFAULT_MAX_COMPRESSED_ANIM_DATA = 75 * 1024 ; 

const int HK_MAX_NUM_DATA_CHUNKS = 10;

/// Base class for all 'animation' jobs.
class hkaAnimationJob
{
	public:
		...
		enum JobType
		{
			...
			ANIMATION_JOB_SAMPLE_ANIMATION = hkJobHelperBaseClass::HK_ANIMATION_JOB_BASE,
...
class hkaSampleAnimationJob : public hkaAnimationJob
{
	public:
		...
		struct AnimationData
		{
			...
			hkaSkeletalAnimation::AnimationType	m_animationType;  
				/// The static animation data required to sample at 'localTime', given by m_frameIndex and m_frameDelta.
			hkaSkeletalAnimation::DataChunk		m_chunks[HK_MAX_NUM_DATA_CHUNKS]; 
...
class hkaAnimationSampleAndCombineJob : public hkaAnimationJob
{
	...
	public:
		...
		hkUint32				m_maxTransformTracksInAllAnimations;
			/// Required to aid reserving of stack space for track sample data.
		hkUint32				m_maxFloatTracksInAllAnimations; 

		/* Output Data */
			/// The transfrom/bone results are placed in this buffer in main memory. If m_parentIndices == HK_NULL, these are in local space, otherwise they are in model space.
		hkQsTransform*			m_poseOut;
			/// The float results are placed in this buffer in main memory.
		hkReal*					m_floatSlotsOut;
			/// This semaphore is released when the work is finished.
