...
class hkpSampledHeightFieldShape : public hkpHeightFieldShape
{
	public:
		...
		virtual hkBool getTriangleFlip() const = 0;
			...
			// Static versions for SPU compatibility. On PPU, these just forward to getHeightAt() / getTriangleFlip().
			// On SPU, they should do whatever DMA'ing might be necessary.
		static HK_FORCE_INLINE hkReal getHeightAtImpl( const hkpSampledHeightFieldShape* hf, int x, int z );
		static HK_FORCE_INLINE hkBool getTriangleFlipImpl( const hkpSampledHeightFieldShape* hf );
	...
	public:
		...
		// Example use case: sampled values for SPU.  NOTE: These members are currently commented
		// out to prevent serialization for recognizing them as members.  To enable heightfields
		// on SPU you also need to comment in these members.
#ifdef HK_ENABLE_HEIGHTFIELDS_ON_SPU
		//mutable hkReal* m_sampledHeights;	//+nosave
		//mutable hkBool* m_sampledFlip;	//+nosave
#endif
