...
#if defined(HK_PLATFORM_SPU)
		// SPU only function which calls the correct ::queryAabb method for the shape provided.
		static hkUint32 queryAabbImpl(const hkpShape* shape, const hkAabb& aabb, hkpShapeKey* hits, int maxNumKeys );

		// SPU only function which calls the correct ::getContainer for the shape provided.
		static HK_SHAPE_CONTAINER* getContainerImpl(const hkpShape* shape);
#endif
